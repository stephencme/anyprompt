...
import { AnyPromptCore } from "../AnyPromptCore"
  ...
  it("should set multiple prompts via setPrompts", () => {
    const prompts = {
      "test1@v1": {
        ...
        name: "test1",
        version: "v1",
        template: "Testing {{ item }} now!",
      },
      "test2@v1": {
        id: "test2@v1",
        name: "test2",
        version: "v1",
        template: "Username is{{user_name  }}",
      },
    }
  ...
  it("should throw an error in setPrompts if one of the templates is invalid", () => {
    const prompts = {
      "test@v1": {
        ...
        name: "good",
        version: "v1",
        template: "This is a good one {{name}}",
      },
      ...
      "bad@v1": {
        ...
        name: "bad",
        version: "v1",
        template: "This is a bad one {{ bad variable }}",
      },
    }

    expect(() => core.setPrompts(prompts)).toThrow()
  })
})
